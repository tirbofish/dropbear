fn main() {
    let jar_path = std::path::PathBuf::from(std::env::var("CARGO_MANIFEST_DIR").unwrap()).join("target/build/libs/dropbear-1.0-SNAPSHOT-all.jar");
    if !jar_path.exists() {
        println!("cargo:warning=Path in {}", jar_path.display());
        build_gradle();
    }
